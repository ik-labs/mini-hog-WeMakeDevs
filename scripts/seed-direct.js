...
/**
 * Direct database seeding script for Docker builds (JavaScript version)
...
function seedEvents() {
  ...
  function insertBatch(startIdx) {
    ...
    const batch = events.slice(startIdx, startIdx + BATCH_SIZE);
    ...
    conn.run(
      ...
      (err) => {
        ...
        process.stdout.write(`\r✓ Inserted ${inserted}/${NUM_EVENTS} events`);
        insertBatch(startIdx + BATCH_SIZE);
      }
    );
  }
...
function verifyCount() {
  conn.all('SELECT COUNT(*) as count FROM events', (err, rows) => {
    if (err) {
